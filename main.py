...
def main() -> None:
    ...
    if not os.path.exists(data_path) or not os.path.exists(item_path):
        ...
        logging.error("Vui lòng kiểm tra lại file tại thư mục data/raw/")
        return

    logging.info("--- BẮT ĐẦU HỆ THỐNG GỢI Ý MOVIELENS 100K (TERMINAL) ---")
    
    # 2. Tải thông tin tên phim từ u.item
    movie_titles = load_movie_titles(item_path)
    logging.info(f"[Bước 1/4] Đã tải thành công danh mục gồm {len(movie_titles)} bộ phim.")

    # 3. Tải ma trận dữ liệu tối ưu (Tận dụng thư mục data/processed/)
    logging.info("[Bước 2/4] Đang nạp ma trận dữ liệu từ thư mục data/processed/...")
    ...
    logging.info(f"-> Ma trận Huấn luyện (Train set): {train_matrix.shape[0]} users x {train_matrix.shape[1]} items.")
    ...
    logging.info("[Bước 4/4 Tiếp theo] Đang thiết lập mô hình Matrix Factorization (SVD)...")
    ...
    if os.path.exists(model_path):
        logging.info("-> Phát hiện file trọng số cũ. Đang nạp nhanh mô hình SVD từ thư mục models/...")
        svd_model.load_model(model_path)
    else:
        logging.info("-> Chưa có file trọng số. Tiến hành huấn luyện mô hình SVD bằng thuật toán SGD (20 epochs)...")
        svd_model.fit(train_matrix)
        logging.info("-> Đang đóng gói và lưu trọng số mô hình vào thư mục models/ để tái sử dụng...")
        svd_model.save_model(model_path)
    
    logging.info("-> Đang kiểm thử hiệu năng SVD...")
    ...
    logging.info("\n" + "="*50)
    logging.info(f"{'KẾT QUẢ ĐÁNH GIÁ ĐỀ TÀI (ĐỘ LỆCH SAI SỐ)':^50}")
    logging.info("="*50)
    ...
    for line in results_df.to_string(index=False).split('\n'):
        logging.info(line)
        
    logging.info("="*50)

    # 8. Kịch bản chạy thử nghiệm gợi ý phim cá nhân hóa cho User cố định trên Terminal
    logging.info("--- ĐANG MÔ PHỎNG GỢI Ý CHO NGƯỜI DÙNG CỤ THỂ ---")
    target_user_id = 1
    target_user_idx = target_user_id - 1
    logging.info(f"Đang tìm danh sách phim gợi ý tối ưu nhất cho User_ID: {target_user_id}...")
    ...
    for item_idx in unviewed_items:
        pred_score = svd_model.predict_rating(target_user_idx, int(item_idx))
    ...
    logging.info(f"Top 5 bộ phim được gợi ý nhiều nhất cho User_ID {target_user_id}:")
